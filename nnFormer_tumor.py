...
class AFNO3D(nn.Module):
    """
    Computes token mixing in the Frequency Domain via 3D FFT.
    (AMP-Compatible Version using view_as_complex)
    """
    def __init__(self, hidden_size, num_blocks=8, sparsity_threshold=0.01):
        ...
        # Define weights as standard float32 with an extra dimension of size 2 for (real, imag)
        # This completely bypasses the AMP GradScaler ComplexFloat error!
        self.w1 = nn.Parameter(torch.randn(num_blocks, self.block_size, self.block_size, 2) * 0.02)
        self.b1 = nn.Parameter(torch.randn(num_blocks, self.block_size, 2) * 0.02)
        self.w2 = nn.Parameter(torch.randn(num_blocks, self.block_size, self.block_size, 2) * 0.02)
        self.b2 = nn.Parameter(torch.randn(num_blocks, self.block_size, 2) * 0.02)

    def forward(self, x, S, H, W):
        B, N, C = x.shape
        x = x.view(B, S, H, W, C).permute(0, 4, 1, 2, 3).contiguous()

        # Convert to Frequency Domain
        x_freq = torch.fft.rfftn(x, dim=(2, 3, 4), norm='ortho')
        S_f, H_f, W_f = x_freq.shape[2], x_freq.shape[3], x_freq.shape[4]

        x_freq = x_freq.view(B, self.num_blocks, self.block_size, S_f, H_f, W_f)
        x_freq = x_freq.permute(0, 3, 4, 5, 1, 2)

        # Convert the float32 parameters to complex ON THE FLY
        w1_c = torch.view_as_complex(self.w1)
        b1_c = torch.view_as_complex(self.b1)
        w2_c = torch.view_as_complex(self.w2)
        b2_c = torch.view_as_complex(self.b2)

        # Apply W1 + B1
        o1 = torch.einsum('bshvni,nio->bshvno', x_freq, w1_c) + b1_c
        
        # Soft-Thresholding
        mag = torch.abs(o1)
        o1 = o1 * (F.relu(mag - self.sparsity_threshold) / (mag + 1e-6))
        
        # Apply W2 + B2
        o2 = torch.einsum('bshvno,noi->bshvni', o1, w2_c) + b2_c

        o2 = o2.permute(0, 4, 5, 1, 2, 3).contiguous() 
        o2 = o2.view(B, C, S_f, H_f, W_f)

        # Inverse FFT
        x_out = torch.fft.irfftn(o2, s=(S, H, W), dim=(2, 3, 4), norm='ortho')
        x_out = x_out.permute(0, 2, 3, 4, 1).contiguous().view(B, N, C)
        
        return x_out

class AFNOBlock(nn.Module):
    """ Mimics the Swin block but replaces Spatial Attention with Frequency AFNO """
    def __init__(self, dim, num_blocks=8, mlp_ratio=4., drop=0., drop_path=0., norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.filter = AFNO3D(dim, num_blocks) # <--- AFNO REPLACES ATTENTION
    ...
    def forward(self, x, S, H, W):
        shortcut = x
        x = self.norm1(x)
        x = self.filter(x, S, H, W)
        x = shortcut + self.drop_path(x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x

class AFNOLayer(nn.Module):
    """ The wrapper layer to replace BasicLayer at the Bottleneck """
    def __init__(self, dim, depth, num_blocks=8, mlp_ratio=4., drop=0., drop_path=0., norm_layer=nn.LayerNorm):
        super().__init__()
        self.blocks = nn.ModuleList([
            AFNOBlock(dim, num_blocks, mlp_ratio, drop, drop_path[i] if isinstance(drop_path, list) else drop_path, norm_layer)
            for i in range(depth)
        ])
        self.downsample = None # No downsampling needed at the deepest bottleneck

    def forward(self, x, S, H, W):
        for blk in self.blocks:
            x = blk(x, S, H, W)
        # Returns standard shape to match BasicLayer signature
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            ...
            # --- START OF AFNO MODIFICATION ---
            if i_layer == self.num_layers - 1:
                # 🚀 The deep Bottleneck Layer uses AFNO Frequency Mixing instead of Swin Attention
                layer = AFNOLayer(
                    dim=int(embed_dim * 2 ** i_layer),
                    depth=depths[i_layer],
                    num_blocks=8, # Splits the 768 bottleneck channels into 8 frequency groups
                    mlp_ratio=mlp_ratio,
                    drop=drop_rate,
                ...
                # Standard Local Window Attention for earlier, high-res layers
                layer = BasicLayer(
                    dim=int(embed_dim * 2 ** i_layer),
                    input_resolution=(
